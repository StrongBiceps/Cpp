...
set,map은 트리 기반이므로 삽입과 삭제 과정에서 벡터와 같이 메모리 재할당이 일어나지 않는다. 따라서 원소들을 가리키던 기존의 반복자는 삭제된 원소를 가리키던 반복자를
제외하면 무효화되지 않는다.

map에는 []연산자로 임의 접근을 사용할 수 있는데 내부 동작은 임의 접근이 아니라 매개변수로 주어진 키를 가지고 레드-블랙 트리를 탐색하는 것이다. 

또한 multimap에서는 []연산자가 정의되어 있지 않은데, 키가 중복되므로 어떤 키에 대한 연산인지 모호하기 때문이다.
...
보통 R-Value인 리터럴 상수 (1,2...)들은 메모리가 할당되지 않고 코드의 해당 부분에 직접 삽입되는 방식이다.

컴파일 시점에 실행 파일에 저장된 5라는 값이 메모리가 할당되지 않고 복사되는 것을 의미한다.

하지만 R-value-Reference가 해당 값을 참조하게 되면 메모리가 할당되고(스택) 주소를 참조하거나 값을 변경할 수 있다.

임시 객체들은 스택에 저장된다.

또한 복잡한 객체나 크기가 큰 문자열 리터럴들은 Stack이나 Data Section 영역에 할당되는데, 

R value Reference가 참조하게 되면 해당 메모리의 주소 공간을 참조하는 것이다.
...
# vtable & vpointer in multiple inheritance

클래스 A와 B가 있고 C가 두 클래스로부터 상속을 받는다고 가정한다. A와 B에는 가상 함수가 존재한다. 다중 상속일 경우에는 각 기반 클래스에 대한 가상 함수 테이블과 가상 함수 포인터가

생성된다. 따라서 C의 크기는 A와 B의 포인터 크기가 합쳐진 크기가 나올 것이다.

32비트 환경에서 포인터의 크기는 4바이트이다.

63비트 환경에서 포인터의 크기는 8바이트이다.
